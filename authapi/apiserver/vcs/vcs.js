...
export class VCS {
  ...
  //Generates a call with n Serial Numbers and returns it as an array of strings 12,34,21,67
  generateCall(n) {
    let encodingLength = Math.max(
      (this.rows - 1).toString().length,
      (this.cols - 1).toString().length
    );
    let result = new Array(n);
    for (let i = 0; i < n; i++) {
      let rowIndex = crypto.randomInt(this.rows).toString();
      let colIndex = crypto.randomInt(this.cols).toString();
      let rowCall = rowIndex.padStart(encodingLength, "0");
      let colCall = colIndex.padStart(encodingLength, "0");
      let call = rowCall + colCall;
      result[i] = call;
    }
    return new Call(encodingLength * 2, result, this.charset);
  }
